...
def test_detect_outliers_iqr(numeric_df):
    outliers = detect_outliers_iqr(numeric_df, "Values")
    assert len(outliers) == 1
    assert outliers.iloc[0]["Values"] == 100

def test_remove_outliers(numeric_df):
    cleaned = remove_outliers(numeric_df, "Values")
...
def test_cap_outliers(numeric_df):
    ...
    cleaned = cap_outliers(numeric_df, "Values")
...
def test_zscore_scale(numeric_df):
    ...
    assert np.isclose(cleaned["Scores"].std(ddof=0), 1.0, atol=1e-7)
